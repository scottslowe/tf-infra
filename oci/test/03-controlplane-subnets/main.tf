...
resource "oci_core_subnet" "subnet_1" {
  compartment_id      = "${var.oci_tenancy_ocid}"
  cidr_block          = "${cidrsubnet(data.terraform_remote_state.vcn.vcn_cidr, 8, 3)}"
  availability_domain = "${lookup(data.oci_identity_availability_domains.ads.availability_domains[0],"name")}"
  vcn_id              = "${data.terraform_remote_state.vcn.vcn_id}"
  display_name        = "${data.terraform_remote_state.vcn.env_name}-cp-subnet-1"
  dns_label           = "cp1"
  route_table_id      = "${data.terraform_remote_state.vcn.priv_rt_id}"
  security_list_ids   = ["${concat(list(data.terraform_remote_state.sec_lists.cp_sec_list_id))}"]
  prohibit_public_ip_on_vnic = true
}

resource "oci_core_subnet" "subnet_2" {
  compartment_id      = "${var.oci_tenancy_ocid}"
  cidr_block          = "${cidrsubnet(data.terraform_remote_state.vcn.vcn_cidr, 8, 4)}"
  availability_domain = "${lookup(data.oci_identity_availability_domains.ads.availability_domains[1],"name")}"
  vcn_id              = "${data.terraform_remote_state.vcn.vcn_id}"
  display_name        = "${data.terraform_remote_state.vcn.env_name}-cp-subnet-2"
  dns_label           = "cp2"
  route_table_id      = "${data.terraform_remote_state.vcn.priv_rt_id}"
  security_list_ids   = ["${concat(list(data.terraform_remote_state.sec_lists.cp_sec_list_id))}"]
  prohibit_public_ip_on_vnic = true
...
resource "oci_core_subnet" "subnet_3" {
  compartment_id      = "${var.oci_tenancy_ocid}"
  cidr_block          = "${cidrsubnet(data.terraform_remote_state.vcn.vcn_cidr, 8, 5)}"
  availability_domain = "${lookup(data.oci_identity_availability_domains.ads.availability_domains[2],"name")}"
  vcn_id              = "${data.terraform_remote_state.vcn.vcn_id}"
  display_name        = "${data.terraform_remote_state.vcn.env_name}-cp-subnet-3"
  dns_label           = "cp3"
  route_table_id      = "${data.terraform_remote_state.vcn.priv_rt_id}"
  security_list_ids   = ["${concat(list(data.terraform_remote_state.sec_lists.cp_sec_list_id))}"]
  prohibit_public_ip_on_vnic = true
